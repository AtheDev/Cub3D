...
/*   Updated: 2021/02/05 10:02:51 by adupuy           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	complete_tab_sprite(t_elts *e)
{
	...
	while (++h < e->height)
	{
		...
		while (++w < e->width)
			...
			{
				num = e->tab[h][w];
				e->tab_sp[ind].num = num;
				e->tab_sp[ind].pos.x = w + 0.5;
				e->tab_sp[ind].pos.y = h + 0.5;
				e->tab_sp[ind].dist = 0.0;
				e->tab_sp[ind].angle = 0.0;
				e->tab_sp[ind].visible = 0;
				ind++;
			}
	}
}
